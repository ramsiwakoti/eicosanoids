...
This notebook will include analysis related to PFAS and Eicosanoids in LIFECODES only
...
pfas_eic_lc <- read.csv("lifecodes//pfas_eic_lc.csv") %>% select(-X)
pfas_eic_lc_imp <- read.csv("lifecodes//pfas_eic_lc_imp.csv") %>% select(-X)
eic_list_new <- read.csv("lifecodes//eic_list_lc_new.csv") %>% select(-X) %>%
  rename("vars" = "analyte")
...
eic_class <- eic_class %>% inner_join(eic_list_new, by = "vars")


# Define a vector for PFAS
exposures_all<- grep("^conc_", names(pfas_eic_lc_imp), value = TRUE)
exposures_short <- c("conc_pfoa", "conc_pfos", "conc_pfhxs", "conc_pfna", "conc_pfdea", "conc_me.pfosa.acoh", "conc_pfua")

# Define a vector for covariates
covars_list <- c("age", "bmi_prepreg", "as.factor(mat_edu_cat)", "as.factor(ins_cat)", "race4", "nullip")
covars_list2 <- c("age", "bmi_prepreg", "as.factor(mat_edu_cat)", "as.factor(ins_cat)", "race4", "nullip", "year_t1")

# Define a vector for eicosanoids
eic_all <- names(pfas_eic_lc_imp[,4:51])

```

### Correlation between visit 1 and visit 3 eicosanoids

```{r correlation between visit 1 and visit 3 eicosanoids}

pfas_eic_lc_imp_wide <- pfas_eic_lc_imp  %>%
  select(id, visit_id, all_of(eic_all), -dipge2, -dipgj2) %>%
  pivot_wider(names_from = visit_id, values_from = dipgf2:x15oxoete, names_prefix = "visit") %>%
  distinct()

cor_all_eic <- cor((pfas_eic_lc_imp_wide %>% select(ends_with("visit1"))), 
                      (pfas_eic_lc_imp_wide %>% select(ends_with("visit3"))), 
                   method = "spearman",
...
### Distributions of eicosanoids by visit 

```{r distributions of eicosanoids by visit}

#Create a long form data
pfas_eic_lc_imp_long <- pfas_eic_lc_imp  %>%
  select(id, visit_id, data, all_of(eic_all), year_t1) %>%
  pivot_longer(cols = dipge2:x15oxoete, names_to = "eic", values_to = "eic_value") 

# Add class label to the dataset

pfas_eic_lc_imp_long2 <- pfas_eic_lc_imp_long %>% 
  inner_join(eic_class, by = c("eic" = "vars"))

#Plot now

pfas_eic_lc_imp_long2 %>% filter(group == "Cyclooxygenase") %>%
  ggplot(aes(y = log(eic_value), x= as.factor(visit_id), col = as.factor(visit_id))) +
  geom_boxplot() +
  facet_wrap(~eic, scales = "free")+
  ggtitle("Cyclooxygenase") +
  theme(legend.position = "none")


pfas_eic_lc_imp_long2 %>% filter(group == "Lipoxygenase") %>%
  ggplot(aes(y = log(eic_value), x= as.factor(visit_id), col = as.factor(visit_id))) +
  geom_boxplot() +
  facet_wrap(~eic, scales = "free")+
  ggtitle("Lipoxygenase") +
  theme(legend.position = "none")

pfas_eic_lc_imp_long2 %>% filter(group == "Cytochrome p450") %>%
  ggplot(aes(y = log(eic_value), x= as.factor(visit_id), col = as.factor(visit_id))) +
  geom_boxplot() +
  facet_wrap(~eic, scales = "free")+
  ggtitle("Cytochrome p450") +
  theme(legend.position = "none")

pfas_eic_lc_imp_long2 %>% filter(group == "Parent Compound") %>%
  ggplot(aes(y = log(eic_value), x= as.factor(visit_id), col = as.factor(visit_id))) +
  geom_boxplot() +
  facet_wrap(~eic, scales = "free")+
  ggtitle("Parent Compound") +
  theme(legend.position = "none")

#Also look at distribution by new vs old data

pfas_eic_lc_imp_long2 %>% filter(visit_id == 3) %>% filter(group == "Cyclooxygenase") %>%
  ggplot(aes(y = log(eic_value), x = eic, col = as.factor(data))) +
  geom_boxplot() +
...
pfas_eic_lc_imp_long2 %>% filter(visit_id == 3) %>% filter(group == "Lipoxygenase") %>%
  ggplot(aes(y = log(eic_value), x = eic, col = as.factor(data))) +
  geom_boxplot() +
  facet_grid(group~., scales = "free")+
  theme(legend.position = "bottom")+
...
pfas_eic_lc_imp_long2 %>% filter(visit_id == 3) %>% filter(group == "Cytochrome p450") %>%
  ggplot(aes(y = log(eic_value), x = eic, col = as.factor(data))) +
  geom_boxplot() +
  facet_grid(group~., scales = "free")+
  theme(legend.position = "bottom")+
...
pfas_eic_lc_imp_long2 %>% filter(visit_id == 3) %>% filter(group == "Parent Compound") %>%
  ggplot(aes(y = log(eic_value), x = eic, col = as.factor(data))) +
  geom_boxplot() +
  facet_grid(group~., scales = "free")+
  theme(legend.position = "bottom")+
...
We see differences in eicosanoids levels by their analysis date. We will have to adjust for data indicator in the model

### Look at eicosanoids by year 

```{r eicosanoids distributions by year}
